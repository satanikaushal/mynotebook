...
router.post("/createuser", [  
  ...
  if (user){ // if the user with the passed email id already exists then then return 400 and the following error message
    success = false
    return res.status(400).json({success,error: "sorry user with these email already exists"})
  }
  ...
  res.json({success,authToken})}).catch((err)=>{
    console.log(err);
    res.json({success,error: "please enter a unique value for email",message : err.message}) ///this will for now return the authtoken as json resolution. and also the errors if we got some.
  })
} catch(error) {
   console.log(error);
   success = false;
   res.status(500).send({success,error:"interanal server error occured"}) // if any error occurs in the try block then it will be catched here and will get it in console as well as the resolution
}
});
...
router.post('/login', [
  ...
  const errors = validationResult(req); // this will again catch the errors that are passed from the above the validations.
   ...
   if (!errors.isEmpty()) {
    success = false
    return res.status(400).json({success, errors: errors.array() }); //also if there are errors then return bad request and error in json format
  }
...
try{
  ...
  success = true; //give them the authtoken so that they can login into there account.
  res.json({success, authtoken}); // for now we will send the authtoken generated as json resolution to this endpoint hit.
}catch(error){
  console.log(error);
  success = false;
  res.status(500).send({success, error: "Internal server error occured"}); // if there are any errors in the try block then catch it here and show the above message.
}
})
